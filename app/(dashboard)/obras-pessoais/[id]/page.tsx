...
import {
  ...
  MapPin, Calendar, FileText, Upload, Loader2, X,
} from "lucide-react";
...
const EXP_STATUS_COLOR: Record<string, string> = { PENDING: "bg-amber-100 text-amber-700", PAID: "bg-green-100 text-green-700", OVERDUE: "bg-red-100 text-red-700" };
const PROV_STATUS: Record<string, string> = { PENDING: "Pendente", IN_PROGRESS: "Em Andamento", COMPLETED: "Concluído", CANCELED: "Cancelado" };
const PROV_STATUS_COLOR: Record<string, string> = { PENDING: "bg-amber-100 text-amber-700", IN_PROGRESS: "bg-blue-100 text-blue-700", COMPLETED: "bg-green-100 text-green-700", CANCELED: "bg-red-100 text-red-500" };
const EXPENSE_CATEGORIES = [
  { value: "material", label: "Material" }, { value: "mao_de_obra", label: "Mão de Obra" },
  { value: "transporte", label: "Transporte" }, { value: "ferramentas", label: "Ferramentas" },
  { value: "alimentacao", label: "Alimentação" }, { value: "outros", label: "Outros" },
];
    ...
    <div className="space-y-5">
      ...
      {s.budgeted > 0 && (
        <Card><CardContent className="pt-4">
          ...
          <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
            <div className={`h-full rounded-full transition-all ${((s.totalCost / s.budgeted) * 100) > 100 ? "bg-red-500" : ((s.totalCost / s.budgeted) * 100) > 80 ? "bg-amber-500" : "bg-blue-500"}`}
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {project.address && <div className="flex items-start gap-2 text-sm"><span className="text-gray-500 w-32 flex-shrink-0">Endereço</span><span className="text-gray-700">{project.address}</span></div>}
          {project.startDate && <div className="flex items-center gap-2 text-sm"><span className="text-gray-500 w-32 flex-shrink-0">Início</span><span className="text-gray-700">{formatDate(project.startDate)}</span></div>}
          {project.expectedEndDate && <div className="flex items-center gap-2 text-sm"><span className="text-gray-500 w-32 flex-shrink-0">Previsão</span><span className="text-gray-700">{formatDate(project.expectedEndDate)}</span></div>}
          {project.description && <div className="flex items-start gap-2 text-sm"><span className="text-gray-500 w-32 flex-shrink-0">Descrição</span><span className="text-gray-700 whitespace-pre-wrap">{project.description}</span></div>}
          {project.notes && <div className="flex items-start gap-2 text-sm"><span className="text-gray-500 w-32 flex-shrink-0">Observações</span><span className="text-gray-700 whitespace-pre-wrap">{project.notes}</span></div>}
        </CardContent>
      </Card>
    </div>
  );
...
function MateriaisTab({ projectId }: { projectId: string }) {
  ...
  const emptyForm = { description: "", supplier: "", quantity: "", unitPrice: "", date: "", paymentMethod: "", attachmentUrl: "" };
  ...
  const { data: materials = [] } = useQuery({
    queryKey: ["personal-materials", projectId],
    queryFn: () => api.personalProjects.listMaterials(projectId) as Promise<any[]>,
  });

  const total = (materials as any[]).reduce((s: number, m: any) => s + Number(m.total), 0);
  const refresh = () => qc.invalidateQueries({ queryKey: ["personal-materials", projectId] });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      if (editId) await api.personalProjects.updateMaterial(projectId, editId, form);
      else await api.personalProjects.createMaterial(projectId, form);
  ...
  const handleEdit = (m: any) => {
    setForm({ description: m.description, supplier: m.supplier ?? "", quantity: String(m.quantity), unitPrice: m.unitPrice != null ? Number(m.unitPrice).toFixed(2) : "", date: m.date ? String(m.date).slice(0, 10) : "", paymentMethod: m.paymentMethod ?? "", attachmentUrl: m.attachmentUrl ?? "" });
    setEditId(m.id); setShowForm(true);
  };

  const handleDelete = async (id: string) => {
    if (!window.confirm("Remover este material?")) return;
    try { await api.personalProjects.deleteMaterial(projectId, id); toast({ title: "Removido", variant: "success" }); refresh(); }
    catch (e: any) { toast({ title: "Erro", description: e.message, variant: "error" }); }
  };
  ...
  const inputCls = "w-full text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-[#EA580C]";
        ...
        <Card><CardContent className="pt-4">
          <form onSubmit={handleSave} className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="sm:col-span-2"><label className="block text-xs font-medium text-gray-600 mb-1">Descrição *</label>
                <input required value={form.description} onChange={f("description")} placeholder="Ex: Cimento CP-II 50kg" className={inputCls} /></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Fornecedor</label>
                <input value={form.supplier} onChange={f("supplier")} placeholder="Nome do fornecedor" className={inputCls} /></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Data</label>
                <input type="date" value={form.date} onChange={f("date")} className={inputCls} /></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Quantidade *</label>
                <input required type="number" step="0.001" min="0" value={form.quantity} onChange={f("quantity")} placeholder="0" className={inputCls} /></div>
              <div><CurrencyInput label="Valor Unitário (R$) *" placeholder="0,00" value={form.unitPrice} onChange={(v) => setForm(prev => ({ ...prev, unitPrice: v }))} /></div>
              ...
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Total</label>
                ...
                {form.attachmentUrl ? (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <button type="button" onClick={() => setForm((prev) => ({ ...prev, attachmentUrl: "" }))}
                      title="Remover nota" className="text-gray-400 hover:text-red-500"><X className="h-3.5 w-3.5" /></button>
                  </div>
                ) : (
        ...
        {(materials as any[]).length === 0 ? <p className="text-sm text-center py-8 text-gray-400">Nenhum material lançado.</p> : (
          <Table>
            <TableHeader><TableRow>
              <TableHead>Descrição</TableHead><TableHead>Fornecedor</TableHead><TableHead>Data</TableHead>
              <TableHead className="text-right">Qtd</TableHead><TableHead className="text-right">Unit.</TableHead>
              <TableHead className="text-right">Total</TableHead><TableHead>Nota</TableHead><TableHead />
            ...
            <TableBody>
              {(materials as any[]).map((m: any) => (
                <TableRow key={m.id}>
                  ...
                  <TableCell>{m.attachmentUrl ? (
                    <a href={m.attachmentUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-[#EA580C] hover:underline"><FileText className="h-3 w-3" />Ver</a>
                  ) : <span className="text-xs text-gray-300">—</span>}</TableCell>
...
function PrestadoresTab({ projectId }: { projectId: string }) {
  ...
  const { data: providers = [] } = useQuery({
    queryKey: ["personal-providers", projectId],
    queryFn: () => api.personalProjects.listProviders(projectId) as Promise<any[]>,
  });
  ...
  const refresh = () => qc.invalidateQueries({ queryKey: ["personal-providers", projectId] });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) await api.personalProjects.updateProvider(projectId, editId, payload);
      else await api.personalProjects.createProvider(projectId, payload);
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await api.personalProjects.deleteProvider(projectId, id); toast({ title: "Removido", variant: "success" }); refresh(); }
    catch (e: any) { toast({ title: "Erro", description: e.message, variant: "error" }); }
  };
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <Card><CardContent className="pt-4">
          <form onSubmit={handleSave} className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Prestador *</label>
                <select required value={form.serviceProviderId} onChange={f("serviceProviderId")} className={inputCls}>
                  <option value="">Selecionar...</option>
                  {serviceProviders.map((sp: any) => <option key={sp.id} value={sp.id}>{sp.name} — {SPECIALTY_LABELS[sp.specialty] ?? sp.specialty}</option>)}
                </select></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Descrição do Serviço *</label>
                <input required value={form.serviceDescription} onChange={f("serviceDescription")} placeholder="Ex: Instalação elétrica" className={inputCls} /></div>
              <div><CurrencyInput label="Valor Combinado (R$)" placeholder="0,00" value={form.agreedAmount} onChange={(v) => setForm(prev => ({ ...prev, agreedAmount: v }))} /></div>
              <div><CurrencyInput label="Valor Pago (R$)" placeholder="0,00" value={form.paidAmount} onChange={(v) => setForm(prev => ({ ...prev, paidAmount: v }))} /></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Vencimento</label><input type="date" value={form.dueDate} onChange={f("dueDate")} className={inputCls} /></div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Data de Pagamento</label><input type="date" value={form.paymentDate} onChange={f("paymentDate")} className={inputCls} /></div>
              ...
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Observações</label>
                <input value={form.notes} onChange={f("notes")} placeholder="Observações..." className={inputCls} /></div>
            </div>
            <div className="flex gap-2 pt-1">
      ...
      <Card><CardContent className="p-0">
        {(providers as any[]).length === 0 ? <p className="text-sm text-center py-8 text-gray-400">Nenhum prestador vinculado.</p> : (
          <Table>
            <TableHeader><TableRow>
              <TableHead>Prestador</TableHead><TableHead>Serviço</TableHead>
              <TableHead className="text-right">Combinado</TableHead><TableHead className="text-right">Pago</TableHead>
              <TableHead className="text-right">Saldo</TableHead><TableHead>Status</TableHead><TableHead />
            </TableRow></TableHeader>
            <TableBody>
              {(providers as any[]).map((p: any) => (
                <TableRow key={p.id}>
                  <TableCell><p className="font-medium text-sm">{p.serviceProvider?.name}</p>
                    {p.serviceProvider?.specialty && <span className={`inline-flex text-xs rounded-full px-1.5 py-0.5 ${SPECIALTY_COLORS[p.serviceProvider.specialty] ?? "bg-gray-100 text-gray-600"}`}>{SPECIALTY_LABELS[p.serviceProvider.specialty]}</span>}
                  </TableCell>
                  ...
                  <TableCell className="text-right text-sm text-amber-700 font-medium">{p.agreedAmount ? formatCurrency(Number(p.agreedAmount) - Number(p.paidAmount ?? 0)) : "—"}</TableCell>
                  <TableCell><span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${PROV_STATUS_COLOR[p.status] ?? "bg-gray-100 text-gray-600"}`}>{PROV_STATUS[p.status] ?? p.status}</span></TableCell>
                  <TableCell><div className="flex items-center justify-end gap-1">
...
function DespesasTab({ projectId }: { projectId: string }) {
  ...
  const { data: expenses = [] } = useQuery({
    queryKey: ["personal-expenses-proj", projectId],
    queryFn: () => api.personalProjects.listExpenses(projectId) as Promise<any[]>,
  });

  const total = (expenses as any[]).reduce((s: number, e: any) => s + Number(e.amount), 0);
  const refresh = () => qc.invalidateQueries({ queryKey: ["personal-expenses-proj", projectId] });

  const handleSave = async (ev: React.FormEvent) => {
    ev.preventDefault(); setSaving(true);
    try {
      const payload = { ...form, amount: form.amount || "0", date: form.date || null };
      if (editId) await api.personalProjects.updateExpense(projectId, editId, payload);
      else await api.personalProjects.createExpense(projectId, payload);
  ...
  const handleEdit = (e: any) => {
    setForm({ description: e.description, category: e.category, amount: e.amount != null ? Number(e.amount).toFixed(2) : "", date: e.date ? String(e.date).slice(0, 10) : "", paymentMethod: e.paymentMethod ?? "", status: e.status });
    setEditId(e.id); setShowForm(true);
  };

  const handleDelete = async (id: string) => {
    if (!window.confirm("Remover esta despesa?")) return;
    try { await api.personalProjects.deleteExpense(projectId, id); toast({ title: "Removida", variant: "success" }); refresh(); }
    catch (e: any) { toast({ title: "Erro", description: e.message, variant: "error" }); }
  };
      ...
      {showForm && (
        <Card><CardContent className="pt-4">
          <form onSubmit={handleSave} className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div><CurrencyInput label="Valor (R$) *" required placeholder="0,00" value={form.amount} onChange={(v) => setForm(prev => ({ ...prev, amount: v }))} /></div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Data</label>
              <input type="date" value={form.date} onChange={f("date")} className={inputCls} /></div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Pagamento</label>
              <select value={form.paymentMethod} onChange={f("paymentMethod")} className={inputCls}>
                <option value="">Selecionar...</option>
                {PAYMENT_METHODS.map(m => <option key={m} value={m}>{m}</option>)}
              </select></div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Status</label>
              <select value={form.status} onChange={f("status")} className={inputCls}>
                <option value="PENDING">Pendente</option><option value="PAID">Pago</option><option value="OVERDUE">Vencido</option>
              </select></div>
            ...
            <TableBody>
              {(expenses as any[]).map((e: any) => (
                <TableRow key={e.id}>
                  ...
                  <TableCell><span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${EXP_STATUS_COLOR[e.status] ?? "bg-gray-100 text-gray-500"}`}>{EXP_STATUS[e.status] ?? e.status}</span></TableCell>
                  <TableCell><div className="flex items-center justify-end gap-1">
...
function HistoricoTab({ project }: { project: any }) {
  const allItems = [
    ...
    ...(project.projectExpenses ?? []).map((e: any) => ({ date: e.createdAt, type: "Despesa", icon: DollarSign, color: "bg-red-100 text-red-700", label: `${e.description} — ${formatCurrency(e.amount)}` })),
    ...(project.projectProviders ?? []).map((p: any) => ({ date: p.createdAt, type: "Prestador", icon: Wrench, color: "bg-blue-100 text-blue-700", label: `${p.serviceProvider?.name} — ${p.serviceDescription}` })),
      ...
      {allItems.map((item, i) => {
        ...
        return (
          <div key={i} className="flex items-start gap-3">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${item.color}`}><Icon className="h-4 w-4" /></div>
...
// ── Main Page ─────────────────────────────────────────────────
export default function ObraPessoalDetailPage(props: { params: Promise<{ id: string }> }) {
  const params = use(props.params);
  const [activeTab, setActiveTab] = useState<Tab>("Resumo");

  const { data: project, isLoading } = useQuery({
    queryKey: ["personal-project", params.id],
    queryFn: () => api.personalProjects.get(params.id) as Promise<any>,
  });
  ...
  return (
    <div className="max-w-5xl space-y-5">
      <PageHeader
        title={project.name}
        description={project.address || "Obra pessoal"}
        actions={
          <div className="flex gap-2">
            <Button variant="outline" size="sm" asChild>
              <Link href="/obras-pessoais"><ArrowLeft className="h-4 w-4" />Voltar</Link>
            </Button>
            <Button variant="outline" size="sm" asChild>
              <Link href={`/obras-pessoais/${params.id}/relatorio`} target="_blank">
                <FileText className="h-4 w-4" />Relatório
              </Link>
            </Button>
            <Button size="sm" asChild>
              <Link href={`/obras-pessoais/${params.id}/editar`}><Pencil className="h-4 w-4" />Editar</Link>
            </Button>
          </div>
        }
      />
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <span className={`inline-flex items-center rounded-full px-3 py-1 text-sm font-medium ${STATUS_COLORS[project.status]}`}>{STATUS_LABELS[project.status]}</span>
