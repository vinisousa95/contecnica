...
export default async function RelatorioObraPessoalPage({ params }: { params: { id: string } }) {
  ...
  const totalProviders = project.projectProviders.reduce((s, p) => s + Number(p.agreedAmount ?? 0), 0);
  const totalPaidProviders = project.projectProviders.reduce((s, p) => s + Number(p.paidAmount ?? 0), 0);
  const totalExpenses = project.projectExpenses.reduce((s, e) => s + Number(e.amount), 0);
      ...
      <body>
        ...
        <div className="page">
          ...
          <div className="header">
            ...
            <div>
              <div className="doc-title">Relatório de Obra Pessoal</div>
          ...
          {project.projectProviders.length > 0 && (
            <div className="section">
              <div className="section-title">Prestadores de Serviço ({project.projectProviders.length})</div>
              <table>
                <thead>
                  <tr>
                    <th>Prestador</th>
                ...
                <tbody>
                  {project.projectProviders.map((p) => {
          ...
          {project.projectExpenses.length > 0 && (
            <div className="section">
              <div className="section-title">Despesas Diversas ({project.projectExpenses.length})</div>
              <table>
                <thead>
                  <tr>
                    <th>Descrição</th>
