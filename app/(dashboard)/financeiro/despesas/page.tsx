...
import { Plus, Search, ArrowDownCircle, Pencil, Trash2 } from "lucide-react";
...
export default function DespesasPage() {
  ...
  const totalShown = expenses.reduce((sum: number, e: any) => sum + e.amount, 0);
    ...
    <div className="space-y-5">
      ...
      {expenses.length > 0 && (
        <div className="text-sm text-gray-500">
          Total exibido:{" "}
          <span className="font-semibold text-gray-800">{formatCurrency(totalShown)}</span>
        </div>
      )}
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <EmptyState
              icon={<ArrowDownCircle className="h-12 w-12" />}
