...
import { Plus, Search, Handshake, Pencil, Trash2, Users } from "lucide-react";
...
export default function ObrasParcelasPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["partnership-projects", search, status, page],
    queryFn: () =>
      api.partnershipProjects.list({
        ...(search && { search }),
        ...(status && { status }),
        page: String(page),
        limit: "20",
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.partnershipProjects.delete(id),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Obras Parcerias"
        description="Obras em parceria com compradores"
        actions={
          <div className="flex gap-2">
            ...
            <Button asChild>
              <Link href="/obras-parcerias/nova">
        ...
        <Card>
          <CardContent className="p-0">
            <EmptyState
              ...
              title="Nenhuma obra parceria encontrada"
              action={
                !search && !status && (
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="min-w-0">
                    <Link href={`/obras-parcerias/${p.id}`} className="font-semibold text-gray-900 hover:text-[#EA580C] text-sm leading-tight block truncate">
                      {p.name}
                    </Link>
                    <p className="text-xs text-gray-500 mt-0.5 truncate">{p.buyer?.name}</p>
                  </div>
                ...
                <div className="flex items-center gap-2 text-xs text-gray-400 mb-4">
                  ...
                  <span>·</span>
                  <span>{p._count?.partnerProviders ?? 0} prestadores</span>
                  <span>·</span>
                  <span>{p._count?.partnerExpenses ?? 0} despesas</span>
                </div>

                <div className="flex items-center justify-between">
                  <Link href={`/obras-parcerias/${p.id}`} className="text-xs text-[#EA580C] hover:underline">
                    Ver detalhes →
                  </Link>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon-sm" asChild title="Editar">
                      <Link href={`/obras-parcerias/${p.id}/editar`}>
